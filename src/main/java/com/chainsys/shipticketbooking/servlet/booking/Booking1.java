	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (aseat >= seat) {
			try {
				...
				RequestDispatcher dispatcher = request.getRequestDispatcher("total1.jsp");
			...
			response.sendRedirect("booking1.jsp?errorMessage=Invalid booking");
		}
	}
}
 ...
 * response.getWriter(); String u =request.getParameter("log"); String p
 * =request.getParameter("pwd"); if(u != "admin" && p!= "admin"){ String
 * someMessage = "Error !";
 * getServletContext().getRequestDispatcher("/index.html").forward(request,
 * response); out.println("<html><head>");
 * 
 * out.println("<script type=\"text/javascript"); out.println("alert("+
 * someMessage +");</script>"); out.println("</head><body></body></html>");
 */